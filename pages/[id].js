...
class SingleCountry extends React.Component {
    ...
    getData = () => {
        ...
        // CURRENCY
        axios.post("/api/currency", {currency: city.local_currency}).then((response) => {
            if (!response.data.error) {
                this.setState({rates: response.data.data});
            }
        });
        // WEATHER
        axios.post("/api/weather", {alias: city.capital_alias}).then((response) => {
            if (!response.data.error) {
                this.setState({
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(SingleCountry));
