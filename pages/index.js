...
class Index extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.post('/api/country', {type: localStorage.getItem('lang') || this.state.type}).then((response) => {
            this.setState({
            ...
            <div className={styles.homepage}>
                <div className={styles.header}>
                    <select onChange={(e) => this.changeLang(e)}
                            value={this.state.type} name="lang" id="lang">
                        <option value={'ru'}>RU</option>
                        <option value={'en'}>EN</option>
                        <option value={'ge'}>GE</option>
                    </select>
                    <Link href="/">Homepage</Link>
                    <input type="text" id={'search'} onKeyDown={(e) => this.searchText(e)} placeholder={'Search'}/>
                    <button onClick={() => this.searchButton()}>Search</button>
                    {!this.state.isAuth ? <div className={styles.auth}>
                        <form
                            id={'Register'}
                        ...
                        <img className={styles.avatar} src={this.state.img} alt="avatar"/>
                        <p>{this.state.name}</p>
                ...
                {this.props.isConnected ? (
                    <>
                        ...
                        <div className={styles.main}>
                            <ul>
                                {this.state[this.state.filteredCountries ? 'filteredCountries' : 'countries'].map((country, index) => {
                                    return (
                                        <div key={index}>
                                            <Link
                                                href={{
                                                    pathname: `/${country.capital_alias}`, query: {
                                                        data: JSON.stringify({
